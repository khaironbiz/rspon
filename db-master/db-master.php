...
<div class="row">
    ...
    $id_master    = $_GET['id'];
    if($id_master !=''){
        $sql_submaster  = mysqli_query($koneksi, "SELECT * FROM db_sub_master where id_master='$id_master' ORDER BY id ASC");
        $sub            = mysqli_fetch_assoc(mysql_query($koneksi, "SELECT * FROM db_master where id='$id_master'"));
    ?>
    <div class="col-lg-4">
        ...
        <div class="table-responsive">
            <table class="table table-striped">
                ...
                <?
                while($data_submaster = mysqli_fetch_assoc($sql_submaster)){
                    $noo++
                ?>
                <tr>
                    <td><? echo $noo?></td>
